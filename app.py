    ...
    if request.method == 'POST':
        try:
            ...
            if not title or not desc:
                return "Title and Description cannot be empty!", 400
...
def update(sno):
    ...
    if request.method == 'POST':
        try:
            ...
            if not todo.title or not todo.desc:
                return "Title and Description cannot be empty!", 400
...
def delete(sno):
    try:
        # Attempt to fetch the Todo item
        todo = Todo.query.get(sno)

        # If the item doesn't exist, handle gracefully
        if not todo:
